...
class Particle:
    ...
    def h_expected_measurement_for_landmark(self, landmark_number,
        ...
        z_expected = self.h(self.pose, self.landmark_positions[landmark_number], scanner_displacement)
        return z_expected  # Replace this.
    ...
    def wl_likelihood_of_correspondence(self, measurement,
        ...
        wl = 1.0/(2.0*pi*sqrt(np.linalg.det(Ql)))*exp(-0.5*a)
        
        return wl # Replace this.
    ...
    def initialize_new_landmark(self, measurement_in_scanner_system,
        ...
        sigma = np.dot(np.linalg.inv(H), np.dot(Qt_measurement_covariance, np.linalg.inv(H).T))
    ...
    def update_landmark(self, landmark_number, measurement,
        ...
        miu_new = self.landmark_positions[landmark_number] + np.dot(K, delta_z)
        sigma_new = np.dot((np.eye(2) - np.dot(K, H)), self.landmark_covariances[landmark_number])
    ...
    def decrement_visible_landmark_counters(self, scanner_displacement):
        ...
        for landmark_number in xrange(self.number_of_landmarks()):
            z_expected = self.h_expected_measurement_for_landmark(landmark_number, scanner_displacement)
